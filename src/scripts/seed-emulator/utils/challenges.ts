...
export async function createUserChallenges(
  ...
  // Create timestamps for challenges completed in the past
  const oneWeekAgo = new Timestamp(now.seconds - 7 * 24 * 60 * 60, 0);
  const twoWeeksAgo = new Timestamp(now.seconds - 14 * 24 * 60 * 60, 0);

  // Generate 20 challenges for each user with different statuses
  // 10 completed, 5 in progress, 5 not started

  // Shuffle the challenge IDs to get random ones
  const shuffledChallengeIds = [...challengeIds].sort(
    () => Math.random() - 0.5
  );

  // Take first 20 challenges
  const selectedChallengeIds = shuffledChallengeIds.slice(0, 20);
  ...
  const completedChallenges = selectedChallengeIds
    ...
    .map((challengeId, index) => {
      ...
      if (index < 3) {
        ...
        // Completed last week
        assignedDate = new Timestamp(
          oneWeekAgo.seconds - index * 12 * 60 * 60,
          0
        );
        ...
        // Completed earlier
        assignedDate = new Timestamp(
          twoWeeksAgo.seconds - (index - 6) * 24 * 60 * 60,
          0
        );
      ...
      // Generate random points (5-30)
      const pointsAwarded = 5 + Math.floor(Math.random() * 26);
